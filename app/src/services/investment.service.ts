...
export const investmentService = {
  ...
  async getMyInvestments(filters?: InvestmentFilters): Promise<PageResponse<Investment>> {
    const params = new URLSearchParams();
    if (filters?.status) params.append('status', filters.status);
    if (filters?.page !== undefined) params.append('page', String(filters.page));
    if (filters?.size !== undefined) params.append('size', String(filters.size));

    const response = await api.get<ApiResponse<PageResponse<Investment>>>(`/investments/my?${params}`);
    return response.data.data;
  },
  ...
  async getPortfolio(): Promise<Portfolio> {
    const response = await api.get<ApiResponse<Portfolio>>('/investments/portfolio');
    return response.data.data;
  },
};
