...
import { useAdminTickets, useUpdateTicketStatus, useUpdateTicketPriority, useAssignTicket, TicketFilters } from '@/hooks/useAdmin';
...
import type { Ticket, TicketStatus, TicketPriority, User as UserType } from '@/types';
...
const statusConfig: Record<TicketStatus, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; color: string }> = {
  OPEN: { label: 'Open', variant: 'default', color: 'bg-blue-500/10 text-blue-400 border-blue-500/30' },
  IN_PROGRESS: { label: 'In Progress', variant: 'secondary', color: 'bg-amber-500/10 text-amber-400 border-amber-500/30' },
  RESOLVED: { label: 'Resolved', variant: 'default', color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' },
  CLOSED: { label: 'Closed', variant: 'outline', color: 'bg-slate-500/10 text-slate-400 border-slate-500/30' },
};
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{tickets?.totalElements || 0}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div>
                ...
                <p className="text-xl font-bold text-blue-400">
                  {filters.status === 'OPEN' || !filters.status
                    ...
                    : tickets?.content.filter((t: Ticket) => t.status === 'OPEN').length || 0}
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div>
                ...
                <p className="text-xl font-bold text-amber-400">
                  {filters.status === 'IN_PROGRESS' || !filters.status
                    ? (tickets?.totalElements || 0)
                    : tickets?.content.filter((t: Ticket) => t.status === 'IN_PROGRESS').length || 0}
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div>
                ...
                <p className="text-xl font-bold text-emerald-400">
                  {filters.status === 'RESOLVED' || !filters.status
                    ? (tickets?.totalElements || 0)
                    : tickets?.content.filter((t: Ticket) => t.status === 'RESOLVED').length || 0}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <Input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && setFilters((prev) => ({ ...prev, page: 0 }))}
                  className="pl-9 bg-slate-800 border-slate-700 text-white"
                />
              </div>
              <Select
                value={filters.status || 'ALL'}
                onValueChange={(v) => setFilters((prev) => ({ ...prev, status: v === 'ALL' ? undefined : v as TicketStatus, page: 0 }))}
              >
              ...
              <Select
                ...
                onValueChange={(v) => setFilters((prev) => ({ ...prev, priority: v === 'ALL' ? undefined : v as TicketPriority, page: 0 }))}
              >
                <SelectTrigger className="w-full md:w-48 bg-slate-800 border-slate-700">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-slate-800">
                    <TableHead className="text-slate-400">Ticket Code</TableHead>
                    <TableHead className="text-slate-400">Subject</TableHead>
                    <TableHead className="text-slate-400">User</TableHead>
                    <TableHead className="text-center text-slate-400">Priority</TableHead>
                ...
                <TableBody>
                  {tickets?.content.map((ticket: Ticket) => {
                    ...
                    return (
                      <TableRow key={ticket.id} className="border-slate-800 hover:bg-slate-800/50">
                        <TableCell className="font-mono text-sm text-white">{ticket.ticketCode}</TableCell>
                        <TableCell>
                          <div className="text-white font-medium">{ticket.subject}</div>
                          <div className="text-xs text-slate-400">{ticket.category}</div>
                        </TableCell>
                        <TableCell>
                          <div className="text-sm text-white">{ticket.user?.fullName || `User #${ticket.userId}`}</div>
                        ...
                        <TableCell className="text-slate-300">
                          {ticket.assignedTo ? (
                            <div className="text-sm">{ticket.assignedTo.fullName}</div>
                          ) : (
            ...
            {selectedTicket && (
              <div className="space-y-2">
                <div className="text-sm text-slate-300">
                  <span className="font-medium">Ticket:</span> {selectedTicket.ticketCode}
                </div>
                <div className="text-sm text-slate-300">
      ...
      {tickets && tickets.totalPages > 1 && (
        <motion.div variants={item} className="flex items-center justify-between">
          <div className="text-sm text-slate-400">
            Showing {filters.page! * filters.size! + 1} to {Math.min((filters.page! + 1) * filters.size!, tickets.totalElements)} of {tickets.totalElements} results
          </div>
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, tickets.totalPages) }, (_, i) => {
                ...
                return (
                  <Button
                    key={pageNum}
                    variant={pageNum === (filters.page || 0) ? 'default' : 'outline'}
                    size="sm"
