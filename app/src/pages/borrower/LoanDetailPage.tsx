...
import { useLoan, useRepaymentSchedule, useCancelLoan, useLoanDocuments, useUploadLoanDocument, useDeleteLoanDocument } from '@/hooks/useLoan';
import { loanService } from '@/services/loan.service';
import { useQuery } from '@tanstack/react-query';
import { formatCurrency, formatDate } from '@/lib/utils';
import { ArrowLeft, Calendar, Percent, Clock, Users, CheckCircle, AlertCircle, X, FileText, Upload, Trash2, Download, Eye } from 'lucide-react';
...
export default function LoanDetailPage() {
  ...
  const canCancel = loan && (loan.status === 'DRAFT' || loan.status === 'PENDING_REVIEW');
  const canUploadDocuments = loan && (loan.status === 'DRAFT' || loan.status === 'PENDING_REVIEW' || loan.status === 'APPROVED');
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="text-2xl font-bold">{formatCurrency(loan.requestedAmount)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
      ...
      {loan.status === 'FUNDING' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {formatCurrency(loan.fundedAmount)} of {formatCurrency(loan.requestedAmount)} funded
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="font-medium">{formatDate(loan.createdAt)}</div>
                </div>
                {loan.disbursedAt && (
                  <div>
                    <div className="text-sm text-muted-foreground">Disbursed At</div>
                    <div className="font-medium">{formatDate(loan.disbursedAt)}</div>
                  </div>
                )}
              </div>
              ...
              <CardContent>
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-bold">
                      {formatCurrency(loan.fundedAmount - (loan.totalRepaid || 0))}
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <TabsContent value="schedule">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Monthly payment schedule for your loan</CardDescription>
            </CardHeader>
            <CardContent>
              {schedules.length === 0 ? (
                          ...
                          <TableCell>
                            {!isPaid && (
                              <Button size="sm" asChild>
                                <Link to={`/dashboard/loans/${loan.id}/repay/${schedule.id}`}>
                                  Pay Now
                                </Link>
                              </Button>
                            )}
        ...
        <TabsContent value="investors">
          <Card>
            <CardHeader>
              ...
              <CardDescription>People who invested in your loan</CardDescription>
            </CardHeader>
            <CardContent>
              {investments.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Total investors: {loan.investorCount || 0}</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {investments.map((investment: any) => (
                      <TableRow key={investment.id}>
                        <TableCell className="font-mono text-sm">{investment.investmentCode}</TableCell>
                        <TableCell className="text-right">{formatCurrency(investment.amount)}</TableCell>
                        <TableCell className="text-right">{investment.interestRate}%</TableCell>
                        <TableCell className="text-right">{formatCurrency(investment.expectedReturn)}</TableCell>
                        <TableCell>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Supporting documents for your loan application</CardDescription>
                </div>
                {canUploadDocuments && (
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <div className="space-y-2">
              ...
              <div className="text-sm">
                <span className="font-medium">Amount:</span> {loan && formatCurrency(loan.requestedAmount)}
              </div>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">Max 50MB. PDF, JPEG, PNG only.</p>
            </div>
            <div className="space-y-2">
