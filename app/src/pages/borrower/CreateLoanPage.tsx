...
import { cn, formatCurrency, calculateLoanMonthlyPayment, calculateTotalInterest } from '@/lib/utils';
...
const loanSchema = z.object({
  ...
  description: z.string()
    ...
    .optional(),
  purpose: z.enum(['PERSONAL', 'BUSINESS', 'EDUCATION', 'MEDICAL', 'HOME_IMPROVEMENT', 'DEBT_CONSOLIDATION', 'STARTUP', 'OTHER'], {
  ...
  amount: z.number({
    ...
    .max(500000000, 'Maximum loan amount is ₫500,000,000'),
  termMonths: z.number({
...
export const CreateLoanPage = () => {
  ...
  const [documents, setDocuments] = useState<Array<{ file: File; type: string; description?: string }>>([]);

  // Estimated interest rate based on credit score (simplified)
  const estimatedRate = user?.creditScore ? Math.max(8, 20 - (user.creditScore / 100)) : 15;
  ...
  const monthlyPayment = calculateLoanMonthlyPayment(watchAmount, estimatedRate, watchTerm);
  const totalInterest = calculateTotalInterest(watchAmount, estimatedRate, watchTerm);
    ...
    if (files && files.length > 0) {
      ...
      Array.from(files).forEach((file) => {
        ...
        if (!allowedTypes.includes(file.type)) {
          alert(`File "${file.name}" is not allowed. Only PDF, JPEG, PNG files are allowed.`);
          return;
        }
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400">Fill in the details to submit your loan application</p>
        </div>
      </div>
      ...
      {user?.kycStatus !== 'APPROVED' && (
        <Card className="bg-amber-500/10 border-amber-500/30">
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              <p className="text-amber-200 font-medium">KYC verification required</p>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">Max 50MB per file • Multiple files allowed</span>
                  </div>
                  
                  <div className="space-y-3">
                    {/* ID Card */}
                    <div className="space-y-2">
                      <Label className="text-sm text-slate-400">ID Card / Passport</Label>
                      ...
                      <p className="text-xs text-slate-500">You can select multiple files</p>
                    </div>

                    {/* Income Proof */}
                    <div className="space-y-2">
                      <Label className="text-sm text-slate-400">Income Proof (Salary Slip, Bank Statement)</Label>
                      <div className="flex items-center gap-2">
                        <Input
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png"
                    ...
                    {/* Other Documents */}
                    <div className="space-y-2">
                      <Label className="text-sm text-slate-400">Other Documents (Business License, etc.)</Label>
                      <div className="flex items-center gap-2">
                        <Input
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png"
                  ...
                  {documents.length > 0 && (
                    <div className="space-y-2 mt-4">
                      <div className="flex items-center justify-between">
                        <Label className="text-sm text-slate-300">Uploaded Documents ({documents.length})</Label>
                        <Button
                          type="button"
                          variant="ghost"
                          size="sm"
                            ...
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-500">
                                  {(doc.file.size / 1024 / 1024).toFixed(2)} MB • {doc.type.replace(/_/g, ' ')}
                                </p>
                              </div>
                            </div>
                            <Button
                ...
                <Button
                  type="submit"
                  disabled={createLoan.isPending || uploadDocument.isPending || user?.kycStatus !== 'APPROVED'}
                ...
                >
                  {createLoan.isPending || uploadDocument.isPending ? (
                    <>
                      ...
                      {uploadDocument.isPending ? 'Uploading documents...' : 'Submitting...'}
                    </>
                  ) : (
                    <>
            ...
            <CardContent className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-800/50 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-medium">{formatCurrency(watchAmount)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 flex items-center gap-2">
                  ...
                  <span className="text-emerald-400 font-medium">{estimatedRate.toFixed(1)}% p.a.</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 flex items-center gap-2">
                  ...
                  <span className="text-white font-medium">{watchTerm} months</span>
                </div>
              </div>
              ...
              <div className="border-t border-slate-700 pt-4 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-amber-400 font-medium">{formatCurrency(totalInterest)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400">Total Repayment</span>
                  <span className="text-emerald-400 font-medium">{formatCurrency(watchAmount + totalInterest)}</span>
                </div>
              </div>

              <p className="text-xs text-slate-500">
                * Interest rate is estimated based on your credit score and may vary after review.
              </p>
            </CardContent>
          </Card>
        </div>
